...
async function main(inputFile) {
    ...
    // list files
    const rootPath = getRootProjectPath(inputFile);
    const rootFiles = fs.readdirSync(rootPath);
    for (const file of rootFiles) {
        if (file.charAt(0) == '.') {
            fs.copySync(file, `${outputPath}/${file}`);
        }
    }
    fs.appendFile(
        Path.normalize(`${outputPath}/.env`),
        `\n\n#Generated by nodeco\nnodeco_secret= ${program.secret}`
    );
...
// return root project that contains package.json
function getRootProjectPath(inputFile) {
    let _path = inputFile;
    if (fs.statSync(inputFile).isFile()) _path = Path.join(inputFile, '..');
    let run = true;
    const limit = 10;
    let i = 0;
    while (run) {
        ...
        if (_path == '/') return inputFile;
        if (i++ == limit) return inputFile;
    }
